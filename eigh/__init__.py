...
def eigh(arr: pyct.NDArray, arg_shape: pyct.NDArrayShape, normalize=True):
    ...
    """
    assert len(np.unique(arg_shape)) == 1, f"arg_shape should be a tuple with the shape of a square matrix (N, N), got {arg_shape}"
    st_sh = arr.shape[:-1]
    st_size = np.prod(st_sh)
    assert st_size * np.prod(arg_shape) == arr.size, f"arg_shape and input array don't coincide. " \
                                                     f"Please provide input of shape  (-1, NxN)"
    N = pycd.NDArrayInfo
    ...
    if N.from_obj(arr) == N.DASK:
        ...
        arr = arr.rechunk(
            chunks=(*((-1),) * len(st_sh), *((-1),) * len(arg_shape))
        )
    else:
...
def eigh_dispatcher(arr, w, v):
    ...
    if N.from_obj(arr) == N.CUPY:
        if d == 2:
            w = compute_eigenvalues_2x2_gpu(arr)
            v = compute_eigenvectors_2x2_gpu(arr, w)
        elif d == 3:
            w = compute_eigenvalues_3x3_gpu(arr)
            v = compute_eigenvectors_3x3_gpu(arr, w)
        else:
        ...
        if d == 2:
            ...
            if N.from_obj(arr) == N.DASK:
                meta = np.array([], dtype=arr.dtype)[
                    (np.newaxis,) * (arr.ndim - 1)
                ]
                tuple_array = arr.map_blocks(
                    np.linalg.eigh, dtype=arr.dtype, meta=meta
                )
